...
{
	private static final int SHOT_SPEED = 200;
	private static final int SHOT_Y_OFFSET = 60;
	private static final float MINIMUM_TIME_BETWEEN_SHOTS = .5f;
	private static final float ENEMY_SHOT_Y_OFFSET = 300;
	private Texture shotTexture;
	private List<AnimatedSprite> shots = new ArrayList<AnimatedSprite>();
	private float timeSinceLastShot=0;
	private Sound laser = Gdx.audio.newSound(Gdx.files.internal("data/laser_bolt.mp3"));
	...
	public void update() 
	{
		...
		while(i.hasNext())
		{
			...
			shot.move();
			if(shot.getY() > AreYouAlive.SCREEN_HEIGHT)
				i.remove();
		}
